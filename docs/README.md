...
- **Duplicate agent directories**: Duplicate underlying agent directories are warned by `syncskill doctor` and are not auto-fixed.
...
- **Install UX**: `syncskill install` with no target shows help; use `syncskill init`, `syncskill install self`, or `syncskill install <url-or-path>` for actual installs, and use `--plan` / `--apply` / `--resolutions` when you need install handoff or approval. See the Usage Guide install section for same-source reuse, subdir installs, and already-installed reporting.
