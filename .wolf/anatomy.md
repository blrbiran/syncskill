...
> Auto-maintained by OpenWolf. Last scanned: 2026-05-06T15:19:00.035Z
> Files: 65 tracked | Anatomy hits: 0 | Misses: 0

## ./

- `.gitignore` — Git ignore rules (~587 tok)
- `AGENTS.md` — AGENTS.md (~268 tok)
...
- `config-ui.ts` — Exports PromptApi, createPromptApi, editAgents, editLinks + 2 more (~1272 tok)
...
- `index.ts` — Exports createProgram (~3672 tok)
...
- `source.ts` — Exports SourceType, SourceDefinition, SourceEntry, SourceState + 7 more (~5234 tok)
...
- `config-ui.test.ts` — Declares PromptStub (~960 tok)
...
- `source.test.ts` — execFileAsync: git, commitAll, createGitSourceFixture + 4 more (~6837 tok)
